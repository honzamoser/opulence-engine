...
export class Renderer extends BaseRenderer {
  ...
  renderEntities(
    ...
    this.device.queue.writeBuffer(
      ...
      0,
      this.projectionMatrix.values as unknown as BufferSource,
    );
    ...
    const modelData = new Float32Array(
      renderableEntities.length * floatsPerEntity,
    );

    for (let i = 0; i < renderableEntities.length; i++) {
      const entity = renderableEntities[i];
      const texture = entity.components.find(
        (c) =>
          c instanceof Texture && c.enabled && c.texture && c.view && c.sampler,
      ) as Texture;
      ...
      modelMatrix = modelMatrix.multiply(
        Matrix4.translation(entity.position.x, entity.position.y, 0),
      );
      modelMatrix = modelMatrix.multiply(Matrix4.rotationZ(entity.rotation));
      modelMatrix = modelMatrix.multiply(
        Matrix4.scaling(entity.scale.x * (flipX ? -1 : 1), entity.scale.y, 1),
      );
      ...
      const animator = entity.components.find(
        (c) => c instanceof Animator,
      ) as Animator;
      ...
      if (animator) {
        ...
        if (texture && texture.image) {
          uvTransform = animator.getUVTransform(
            texture.image.width,
    ...
    for (let i = 0; i < renderableEntities.length; i++) {
      const entity = renderableEntities[i];
      const textureComponent = entity.components.find(
        (c) =>
          c instanceof Texture && c.enabled && c.texture && c.view && c.sampler,
      ) as Texture;
  ...
  createSingleAttributeVertexBuffer(
    vertices: Float32Array,
    attributeDescription: GPUVertexAttribute,
    arrayStride: number,
  ) {
    if (!this.device) {
  ...
  render(
    ...
    const { buffer: positionBuffer, layout: positionBufferLayout } =
      this.createSingleAttributeVertexBuffer(
        vertices,
        {
          format: "float32x3",
          offset: 0,
    ...
    const { buffer: colorBuffer, layout: colorBufferLayout } =
      this.createSingleAttributeVertexBuffer(
        colors,
        {
          format: "float32x3",
    ...
    passEncoder.setPipeline(
      this.createPipeline(
        this.createShaderModule(this.device, shaderCode),
        [positionBufferLayout, colorBufferLayout],
        format,
      ),
    );
