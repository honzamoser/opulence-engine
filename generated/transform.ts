...
export class TransformComponent {
    ...
    static initialize(v: {vf32: Float32Array,vi32: Int32Array,vu8: Uint8Array}) {
		TransformComponent.vf32 = v.vf32
		TransformComponent.vi32 = v.vi32
		TransformComponent.vu8 = v.vu8


        TransformComponent.IS_INITIALIZED = true;
    }
    static new(v: Partial<TransformComponentSignature>) {
        const memId = TransformComponent.SET.length;
        TransformComponent.SET[memId] = memId;
...
TransformComponent.vf32.set(constructionData.matrix, base / 4 + 10);
    ...
    static delete() {
       if (TransformComponent.CURSOR < TransformComponent.SET.length) {
        ...
        if (TransformComponent.SET.length > TransformComponent.MEM_CURSOR) {
            const baseSrc = TransformComponent.NEXT * 108;
            const baseDst = TransformComponent.MEM_CURSOR * 108;

            // Copy data in vf32
            for (let i = 0; i < 120 / 4; i++) {
                TransformComponent.vf32[baseDst / 4 + i] = TransformComponent.vf32[baseSrc / 4 + i];
            }
        }
    }
    ...
    static to(cId: number) {
        ...
        if (TransformComponent.SET[cId] == undefined) {
            throw new Error("Entity does not have this component");
        }

        TransformComponent.MEM_CURSOR = TransformComponent.SET[cId]
