...
export class Renderer3D extends BaseRenderer {
  ...
  createBuffers() {
    ...
    const vertices = new Float32Array([
      // x, y, z,    u, v
      -0.5, 0.0, -0.5, 0.0, 0.0, -0.5, 0.0, 0.5, 0.0, 1.0, 0.5, 0.0, 0.5, 1.0,
  ...
  createUniforms() {
    ...
    this.projectionViewBindGroup = this.device.createBindGroup({
      ...
      entries: [
        { binding: 0, resource: { buffer: this.projectionViewBuffer } },
      ],
    });
  }
...
`;
  ...
  updateProjectionView(matrix: Matrix4) {
    ...
    this.device.queue.writeBuffer(
      ...
      0,
      matrix.values as unknown as ArrayBuffer,
    );
  }
  ...
  render(instances: { model: Matrix4; textureBindGroup: GPUBindGroup }[]) {
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      this.device.queue.writeBuffer(
        this.modelBuffer,
        off,
        instances[i].model.values as unknown as ArrayBuffer,
      );
      const modelBindGroup = this.device.createBindGroup({
        ...
        entries: [
          {
            binding: 0,
            resource: { buffer: this.modelBuffer, size: 64, offset: off },
          },
        ],
      });
  ...
  renderEntities(entities: Entity[], cameraPosition?: Vector2): void {
    ...
    const meshInstances: {
      model: Matrix4;
      modelComponent: Model3D;
      textureBindGroup?: GPUBindGroup;
    }[] = [];

    for (const e of entities) {
      const modelComp = e.components.find((c) => c instanceof Model3D) as
        | Model3D
        | undefined;
      const tex = e.components.find((c) => c instanceof Texture) as
        | Texture
        | undefined;
      ...
      if (modelComp && modelComp.positionBuffer) {
        ...
        model = model.multiply(
          Matrix4.translation(e.position.x, 0, e.position.y),
        );
        ...
        meshInstances.push({
          ...
          textureBindGroup: tex ? tex.bindGroup! : undefined,
        });
        continue;
      }
  ...
  renderMeshes(
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      this.device.queue.writeBuffer(
        this.modelBuffer,
        off,
        inst.model.values as unknown as ArrayBuffer,
      );
      const modelBindGroup = this.device.createBindGroup({
        layout: this.modelBindGroupLayout!,
        entries: [
      ...
      if (comp.indexBuffer)
        pass.setIndexBuffer(comp.indexBuffer, comp.indexFormat);
