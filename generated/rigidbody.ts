...
export class RigidbodyComponent {
    ...
    const constructionData: RigidbodyComponentSignature = {
        velocity: v.velocity ? v.velocity : vec3.create(),
...
static get velocity() {
            return RigidbodyComponent.vf32.subarray((4 / 4) + (32 / 4) * RigidbodyComponent.MEM_CURSOR, (4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 3)
    }
            
    static set velocity(v: Vec3 | Float32Array) {
        RigidbodyComponent.vf32.set(v, (4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR);
    }
        
    static cpy_velocity(out: Vec3) {
        out.set(RigidbodyComponent.vf32.subarray((4 / 4) + (32 / 4) * RigidbodyComponent.MEM_CURSOR, (4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 3));
    }
        
    static get velocityX() {
        return RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 0];
    }

    static set velocityX(v: number) {
        RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 0] = v;
    }
static get velocityY() {
        return RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 1];
    }

    static set velocityY(v: number) {
        RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 1] = v;
    }
static get velocityZ() {
        return RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 2];
    }

    static set velocityZ(v: number) {
        RigidbodyComponent.vf32[(4 / 4) + 32 / 4 * RigidbodyComponent.MEM_CURSOR + 2] = v;
    }

static get mass() {
            return RigidbodyComponent.vf32[4 + 32 / 4 * RigidbodyComponent.MEM_CURSOR]
        } 
            
        static set mass(v: number) {
            RigidbodyComponent.vf32[4 + 32 / 4 * RigidbodyComponent.MEM_CURSOR] = v;
        }

static get bodyId() {
            return RigidbodyComponent.vf32[5 + 32 / 4 * RigidbodyComponent.MEM_CURSOR]
        } 
            
        static set bodyId(v: number) {
            RigidbodyComponent.vf32[5 + 32 / 4 * RigidbodyComponent.MEM_CURSOR] = v;
        }

static get isStatic() {
            return RigidbodyComponent.vu8[24 + 32 * RigidbodyComponent.MEM_CURSOR] === 1;
        }

        static set isStatic(v: boolean) {
            RigidbodyComponent.vu8[24 + 32 * RigidbodyComponent.MEM_CURSOR] = v ? 1 : 0;
        }

static get _componentId() {
            return RigidbodyComponent.vf32[0 + 32 / 4 * RigidbodyComponent.MEM_CURSOR]
        } 
            
        static set _componentId(v: number) {
            RigidbodyComponent.vf32[0 + 32 / 4 * RigidbodyComponent.MEM_CURSOR] = v;
        }
